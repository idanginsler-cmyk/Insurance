...
5. **Typography consistency** — ניתוח עקביות טיפוגרפית **בתוך מסמך אחד**, ללא
   צורך בקבלה מקורית להשוואה. עבור כל שורת ספרות (סכומים, תאריכים, מספרי
   קבלה) מחושבים ה־z-score של גובה, baseline, kerning, חדות (Laplacian
   variance) ובהירות ביחס לשאר התווים בשורה. תו עם **שתי מטריקות או יותר
   שמתואמות** מעל הסף = חשד גבוה לעריכה ידנית (פונט/גודל אחר). תופס במיוחד
   את המקרה של "1 שהוסף לפני 600 → 1600". משתמש ב־`pytesseract.image_to_boxes`
   לתיבות char-level אמיתיות; fallback לפיצול תיבות מילים.
6. **Field extraction** — ספק, תאריך (DD/MM/YYYY ישראלי), סכום (כולל זיהוי ₪),
   מספר קבלה. רגקס + עוגני מילות מפתח בעברית.
7. **Text fingerprinting** — חתימה קנונית `provider|date|amount|receipt#`
   להתאמה מדויקת + Jaccard על טוקנים נורמליים להתאמה רכה (שורד גם
   re-photograph של אותה קבלה).
8. **Optional CLIP embeddings** — לדמיון סמנטי מעבר ל־hash. מושבת כברירת
   מחדל כי דורש torch ~2GB.
9. **Ensemble scoring** — ציון 0–100 + רמת סיכון + רשימת הסיבות
...
```bash
make demo               # חתימות תמונה: phash + duplicate detection
make demo-typography    # זיהוי "1" שהוסף ל"600" — ללא צורך בהשוואה
```

`make demo` יוצר 4 קבלות סינתטיות (אותנטית, הגשה־חוזרת ביט־לביט, סכום מזויף
600→1600, ספק לא קשור), מזין אותן, ומנתח את ה־resubmission. ציפייה:
phash_exact match, רמה HIGH/CRITICAL.

`make demo-typography` רץ על שתי קבלות (clean, forged) ב־**מאגר ריק** — אין
מסמך מקור להשוואה. הציפייה: forged מקבל MEDIUM (ציון ~33) על בסיס חוסר
עקביות טיפוגרפית בלבד; clean נשאר CLEAN.
